...
interface DataContextType {
  ...
  getVideoCategories: () => Promise<{ assignments: Record<string, string[]> }>
...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const getVideoCategories = useCallback(async (): Promise<{ assignments: Record<string, string[]> }> => {
    return apiFetch('/video-categories')
  }, [])

  const saveVideoCategories = useCallback(async (videoId: string, categories: string[]): Promise<any> => {
    return apiFetch('/video-categories', { method: 'POST', body: JSON.stringify({ videoId, categories }) })
  }, [])

  const getFolderVideos = useCallback(async (folder: string): Promise<{ folder: string; videos: any[] }> => {
    return apiFetch(`/folder-videos?folder=${encodeURIComponent(folder)}`)
  }, [])

  const saveFolderVideos = useCallback(async (folder: string, videos: any[]): Promise<any> => {
  ...
  const getFolderThumbnails = useCallback(async (): Promise<{ thumbnails: Record<string, string> }> => {
    return apiFetch('/folder-thumbnails')
  }, [])

  const saveFolderThumbnail = useCallback(async (folder: string, thumbnailUrl: string): Promise<any> => {
